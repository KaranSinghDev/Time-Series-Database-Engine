...
graph TD;
    subgraph "Backend Service (Docker Container)"
        FastAPI[Python FastAPI Server] -- "Loads" --> CppEngine[libinsight.so];
        subgraph "C++ Storage Engine (libinsight.so)"
            CppEngine -- "Writes/Reads" --> Shards[Time-Sharded .bin Files];
        end
    end

    subgraph "Frontend Client (Local Process)"
        Dashboard[PyQt6 GUI Client]
    end

    Dashboard -- "HTTP API Calls (requests)" --> FastAPI;
